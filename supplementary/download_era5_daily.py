...
# Defines function for retrieving daily ERA5* via CDS API.
# 
# Original script by Kevin Marsh:
...
def retrieve_era5_daily(
    ...
    if dataset == 'reanalysis-era5-pressure-levels':
        assert (pressure_level >= 1) & (pressure_level <= 1000)
    ...
    pressure_level = '' if pressure_level == '-' else pressure_level + '_'
    file_name = file_name if file_name else dataset + '_' + variable + '_' + pressure_level + statistic + '_' + year + '_' + month + '.nc'
    if folder:

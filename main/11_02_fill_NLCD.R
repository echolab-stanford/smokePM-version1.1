...
# Fills NLCD by nearest non-missing neighbor.
#-------------------------------------------------------------------------------
...
path_filled = file.path(path_data, "2_from_EE", "NLCD_1km_subgrid_filled")
if (!dir.exists(path_filled)) dir.create(path_filled)
files = list.files(file.path(path_data, "2_from_EE", "NLCD_1km_subgrid"), full.names = T)
...
# Save
write.csv(nlcd_filled, 
          file = file.path(path_data, "2_from_EE", "NLCD_areas_10km_grid_filled.csv"),
          row.names = F)

...
if(!dir.exists(file.path(path_output_sherlock, "smokePM_interpolation", "smoke_grid_cell"))){
  dir.create(file.path(path_output_sherlock, "smokePM_interpolation", "smoke_grid_cell"), recursive = T)
}
...
# station_full_panel <- readRDS("~/BurkeLab Dropbox/projects/smokePM-prediction/data/3_intermediate/station_smokePM.rds") %>% ungroup
# station_full_panel %<>% mutate(id_orig = id, id = formatC(id, width = 9, flag =  "0"))
# station_full_panel %<>% filter(year == 2006)
...
interp_parameters <- readRDS(file.path(path_output_sherlock, "smokePM_interpolation",
                                       "interp_tune_params_drop9999.rds")) %>%
...
map_chr(year_months, function(y_m){
  ...
  out_file <- file.path(path_output_sherlock, "smokePM_interpolation", "smoke_grid_cell", 

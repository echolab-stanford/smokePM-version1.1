...
end_month = "2024-12" # format(today() - months(2), "%Y-%m")
#-#-----------------------------------------------------------------------------
...
interp_all = expand.grid(x = c(1:5), 
  ...
  pmap(function(x, y){
    ...
    readRDS(file.path(path_output_sherlock, sprintf("version%s", model_version), "smokePM_interpolation", "revisions", "best_interp",
                      paste0("interp_drop", paste0(j, collapse = ""), "_200601_202412.rds"))) %>% 
...
smokePM_training %>% 
  ...
  drop_na(smokePM) %>% 
  saveRDS(file.path(path_output_sherlock, sprintf("version%s", model_version), "smokePM", "revisions", "smokePM_full_model_training.rds"))
